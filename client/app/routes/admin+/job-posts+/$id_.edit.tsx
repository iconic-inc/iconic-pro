...
export const loader = async ({ request, params }: LoaderFunctionArgs) => {
  ...
  const ownersPromise = listJobPosts(
    ...
    console.error('Error loading job post owners:', error);
    return {
      data: [],
  ...
  return { jobPost, ownersPromise };
};

export default function EmpNewJobPost() {
  const { jobPost, ownersPromise } = useLoaderData<typeof loader>();

  return (
    <>
      <DashContentHeader title='Thay đổi thông tin Tin tuyển dụng' />

      <JobPostEditForm jobPost={jobPost} ownersPromise={ownersPromise} />
    </>
  );
}
