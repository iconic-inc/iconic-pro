...
export const loader = async ({ request, params }: LoaderFunctionArgs) => {
  ...
  try {
    ...
    const { id } = params;
    if (!id) {
      // Nếu không có ID, chuyển hướng về trang danh sách Ứng viên
    ...
    return {
      ...
      error:
        error instanceof Error
          ? error.message
          : 'Đã xảy ra lỗi khi tải dữ liệu Ứng viên',
    };
  }
};
...
export default function EmpCandidateDetail() {
  ...
  const { candidate, error } = useLoaderData<typeof loader>();
  const navigate = useNavigate();

  // Kiểm tra nếu có lỗi hoặc không có dữ liệu Ứng viên
  if (error || !candidate) {
    return (
      <>
        {/* <ContentHeader title='Chi Tiết Ứng viên' /> */}
