...
export default function CandidateConfirmModal({
  setShowDeleteModal,
  selectedCandidates,
  ...
  useEffect(() => {
    if (bulkDeleteFetcher.data) {
      ...
      if (response.success) {
        toast.success(
          `Đã xóa ${selectedCandidates.length} Ứng viên thành công!`,
        );
        setSelectedCandidates([]);
        // Refresh dữ liệu
        window.location.reload();
      } else {
        toast.error(response.error || 'Có lỗi xảy ra khi xóa Ứng viên.');
      }
      setIsDeleting(false);
    }
      ...
      >
        ...
        <p className='mb-6'>
          {selectedCandidates.length > 1
            ? `Bạn có chắc chắn muốn xóa ${selectedCandidates.length} Ứng viên?`
